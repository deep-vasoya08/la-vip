...
/**
 * Handle Stripe refund-related webhook events for tour bookings
 ...
 */
export async function handleTourRefund(eventData: RefundEventInput) {
  ...
  try {
    ...
    if (isCharge(eventData)) {
      ...
      console.log('Processing tour refund webhook from charge:', eventData.id)
      ...
      console.log('Processing tour refund webhook from refund update:', eventData.id)
      paymentIntentId = eventData.payment_intent as string
      refund = eventData
    }
    ...
    if (payments.docs.length === 0) {
      console.log(`No tour payment found for payment intent: ${paymentIntentId}`)
      return
    }
    ...
    console.log(`Processing tour refund: ${refund.id} for payment: ${payment?.id}`)
      ...
      if (booking?.user) {
        ...
        if (userEmail) {
          try {
            ...
            if (typeof booking.tour === 'object' && booking.tour?.name) {
              bookingName = booking.tour.name
            }
            ...
            if (emailResult.success) {
              console.log('Tour refund confirmation email sent successfully')
            } else {
              console.error('Failed to send tour refund confirmation email')
            }

            // Console logging with refund details
            console.log('\n' + '='.repeat(60))
            console.log('💸 TOUR REFUND PROCESSED!')
            ...
            console.log('\n🔗 QUICK ACCESS LINKS:')
            console.log(`📊 Admin Booking: ${baseUrl}/admin/collections/tour_bookings/${bookingId}`)
            console.log(`💳 Payment Details: ${baseUrl}/admin/collections/tour_booking_payments`)
            ...
            console.error('Error sending tour refund confirmation email:', error)
          }
        }
      }
    }
  } catch (error) {
    console.error('Error processing tour refund webhook:', error)
    throw error
  }
}
