...
/**
 * Utility function to revalidate the events sitemap cache
 * Can be called from anywhere in the application
 */
export const revalidateEventsSitemapCache = async () => {
  try {
    revalidateTag('events-sitemap')
    console.log('🔄 Events sitemap cache revalidated')
    return { success: true }
  } catch (error) {
    console.error('❌ Error revalidating events sitemap cache:', error)
 ...
 */
export const triggerEventsSitemapRevalidation = async (baseUrl?: string) => {
  try {
    const url = baseUrl || process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:3000'
    const response = await fetch(`${url}/api/revalidate-events-sitemap`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      console.log('🔄 Events sitemap revalidated via API:', result)
