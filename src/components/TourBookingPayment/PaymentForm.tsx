...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const { trackTourBeginCheckout, trackTourAddPaymentInfo, trackTourPurchase } = useGA4Tracking()
  const {
    trackTourInitiateCheckout: trackMetaTourInitiateCheckout,
    trackTourAddPaymentInfo: trackMetaTourAddPaymentInfo,
    trackTourPurchase: trackMetaTourPurchase,
  ...
  useEffect(() => {
    ...
    if (!bookingCreationInProgress && !bookingId && !clientSecret && tourData) {
      const initializePaymentProcess = async () => {
        ...
        try {
          ...
          console.log('Creating booking with tour data:', tourData)
          const bookingResponse = await fetch('/api/bookings/tours', {
            method: 'POST',
          ...
          try {
            const tourForTracking = {
              ...
              name: 'Tour Booking',
              tourType: 'General',
              pickups: [{ adult_price: amount }],
            }
            const bookingForTracking = {
              adultCount: tourData.adultCount || 1,
              childCount: tourData.childCount || 0,
              bookingReference: bookingResult.booking.bookingReference,
              scheduledDate: tourData.tourDateTime,
              pickupDetails: { hotelId: tourData.pickupLocationId },
            }
            ...
            trackTourBeginCheckout(
              tourForTracking as any,
              bookingForTracking as any,
              pricingForTracking,
            )

            // Track Meta Pixel
            trackMetaTourInitiateCheckout(
              tourForTracking as any,
              bookingForTracking as any,
              pricingForTracking,
            )
          } catch (trackingError) {
            // Silently handle tracking errors in production to not affect user experience
          }

          // console.log('Fetching payment intent for booking:', newBookingId)
          const paymentResponse = await fetch('/api/payments/tours', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
  ...
  }, [tourData, amount, bookingId, clientSecret, bookingCreationInProgress])
  ...
  const CheckoutForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      if (result.error) {
        ...
        if (bookingDetails) {
          try {
            ...
            trackTourPurchase(
              ...
              bookingDetails.tour as any,
              bookingDetails,
              bookingDetails.pricing,
            )

            // Track Meta Pixel
            trackMetaTourPurchase(
        ...
        if (bookingId) {
          onSubmit?.(result.paymentIntent.id)
        }
        setIsProcessing(false)
      }
    }
